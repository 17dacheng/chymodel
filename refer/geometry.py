...
class InterfaceGraph(nn.Module, core.Configurable):
    ...
    def forward(self, graph):
        # 保存原始关键属性
        original_attrs = {
            'num_cum_residues': getattr(graph, 'num_cum_residues', None),
            'num_residue': getattr(graph, 'num_residue', None)
        }
        ...
        if self.entity_level in ["node", "atom"]:
            ...
            new_graph = graph.subresidue(mask)
        
        # 确保关键属性存在
        if not hasattr(new_graph, 'num_cum_residues') and original_attrs['num_cum_residues'] is not None:
            new_graph.num_cum_residues = original_attrs['num_cum_residues']
        
        if not hasattr(new_graph, 'num_residue') and original_attrs['num_residue'] is not None:
            new_graph.num_residue = original_attrs['num_residue']
