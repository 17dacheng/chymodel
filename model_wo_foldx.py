...
class CHYModelWithGeometric(nn.Module):    
    def __init__(self, 
        ...
        # 特征融合注意力权重（只有序列和几何特征两个权重）
        self.feature_fusion = nn.Sequential(
            nn.Linear(
                hidden_dim // 4 + hidden_dim // 4,  # seq + antisymmetric = 128 + 128 = 256
                hidden_dim // 2  # 中间层维度
            ),
            nn.LayerNorm(hidden_dim // 2),
            nn.ReLU(),
    ...
    def forward(self, 
        ...
        Args:
            ...
            foldx_features: [batch_size, foldx_dim] FoldX能量项（不使用）
        ...
        weighted_features = (
            fusion_weights[:, 0:1] * seq_rep +  # 序列特征权重
...
def load_pretrained_geometric_model_wo_foldx(model_path: str, device: str = 'cpu') -> CHYModelWithGeometric:
    """加载预训练模型（无FoldX版本）"""
    ...
    model.eval()
    model.to(device)
    print(f"从 {model_path} 加载预训练模型成功（无FoldX版本）")
    return model
...
if __name__ == "__main__":    
    ...
    if Path(csv_path).exists():
        ...
        # 保存结果
        tester.save_results(csv_results, "test_results_s1131_wo_foldx.csv")
        print(f"结果已保存到: test_results_s1131_wo_foldx.csv")
    else:
